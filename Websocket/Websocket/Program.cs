...
namespace Websocket
{
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            var tServer = new TextServer();
            tServer.Start("http://localhost:44300/socket/");

            var bServer = new ByteServer();
            bServer.Start("http://localhost:44400/socket/");
    ...
    {
        private int count = 0;
        List<WebSocket> onChat = new List<WebSocket>();
        //### Starting the server        
        ...
        private async void ProcessRequest(HttpListenerContext listenerContext)
        {
            ...
            try
            {
                ...
                {
                    if(!onChat.Contains(webSocket))
                        onChat.Add(webSocket);
                    Console.WriteLine("Text recieved");
                    ...
                    {
                        foreach(WebSocket socket in onChat)
...
//.ripple-effect{-webkit-animation:ripple .8s ease-out;animation:ripple .8s ease-out}
